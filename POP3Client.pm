...
sub Connect
{
	...
	$msg = <$s> or $me->Message("Could not read") and return 0;
	chop $msg;
	$me->Message($msg);
...
sub Login
{
	...
	chop;
	$me->Message($_);
	/^\+/ or $me->Message("USER failed: $_") and $me->State('AUTHORIZATION')
		and return 0;
	...
	chop;
	$me->Message($_);
	/^\+/ or $me->Message("PASS failed: $_") and $me->State('AUTHORIZATION')
		and return 0;
...
sub Head
{
	...
	$me->Debug and print;
	chop;
	/^\+OK/ or $me->Message("Bad return from TOP: $_") and return '';
	/^\+OK (\d+) / and $buflen = $1;
...
sub HeadAndBody
{
	...
	$me->Debug and print;
	chop;
	/^\+OK/ or $me->Message("Bad return from RETR: $_") and return '';
	/^\+OK (\d+) / and $buflen = $1;
...
sub POPStat {
	...
	print $s "STAT", $me->EOL;
	$_ = <$s>;
	/^\+OK/ or $me->Message("STAT failed: $_") and return 0;
...
sub Retrieve {
	...
	print $s "RETR $num", $me->EOL;
	$_ = <$s>;
	/^\+OK\s*/ or $me->Message($_) and return;

	while($_ = <$s>) {
		$me->Debug and print STDERR "DEBUG: $_";
		/^\.\s*$/ and last;
		$ret .= $_;
		chop;
		push(@retarray, $_);
